...
// ---------------------------------------------------------------------------
// Real USDC movement for the autonomous agent.
...
import {
  CIRCLE_BLOCKCHAIN,
  NETWORK_LABEL,
  explorerTxUrl as _explorerTxUrl,
} from '@/lib/arc-network';

/** @deprecated Import from '@/lib/arc-network' instead. Re-exported for back-compat. */
export const AGENT_BLOCKCHAIN = CIRCLE_BLOCKCHAIN;
export const AGENT_WALLET_SET_NAME = 'ENTARC Agent';

/** @deprecated Import from '@/lib/arc-network' instead. Re-exported for back-compat. */
export function explorerTxUrl(txHash: string): string {
  return _explorerTxUrl(txHash);
}

/**
 * Find (or provision on first run) the agent's wallet on the active Arc network.
...
export async function getOrCreateAgentWallet(): Promise<CircleWallet> {
  ...
  const existing = list.find((w) => w?.blockchain === CIRCLE_BLOCKCHAIN);
...
export async function transferUsdc(input: {
  ...
  return {
    ...
    state,
    txHash,
    explorerUrl: txHash ? _explorerTxUrl(txHash) : undefined,
  };
}
