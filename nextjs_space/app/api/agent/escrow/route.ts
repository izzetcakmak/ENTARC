 ...
 */
async function handleFundProposal(body: any, phase: 'initial' | 'milestone', session: any) {
  ...
  if (proposalId) {
    ...
    // On mainnet, restrict direct mode to ADMIN users only.
    if (IS_MAINNET) {
      ...
      if (caller?.role !== 'ADMIN') {
        return NextResponse.json(
          { error: 'Direct escrow transfers are disabled on mainnet for non-admin users.' },
          { status: 403 }
        );
      }
    }
...
async function handleCheckStatus() {
  ...
  return NextResponse.json({
    success: true,
    status: {
      agentWallet: { id: wallet.id, address: wallet.address, blockchain: CIRCLE_BLOCKCHAIN },
