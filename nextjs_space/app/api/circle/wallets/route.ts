...
export async function GET(req: NextRequest) {
  try {
    ...
    if (status !== 401) console.error('Circle wallets error:', error);
    return NextResponse.json(
      { error: status === 401 ? 'Circle API credentials invalid or expired' : (error.message || 'Failed to fetch wallets'), success: false, wallets: [], walletSets: [] },
      { status }
    );
  }
}
  ...
  try {
    ...
    if (action === 'create-wallet') {
      ...
      const res = await client.createWallets({
        walletSetId,
        blockchains: [blockchain || (await import('@/lib/arc-network')).CIRCLE_BLOCKCHAIN],
        count: 1,
        accountType: 'EOA',
      });
