...
export async function POST(req: NextRequest) {
  try {
    ...
    // Faucet is testnet-only. Block on mainnet.
    const { IS_MAINNET, CIRCLE_BLOCKCHAIN } = await import('@/lib/arc-network');
    if (IS_MAINNET) {
      return NextResponse.json(
        { error: 'Faucet is not available on mainnet. Fund wallets through Circle Console or a bridge.' },
        { status: 400 }
      );
    }

    const body = await req.json();
    const { address, blockchain = CIRCLE_BLOCKCHAIN, usdc = true, native = true } = body;
    ...
    if (faucetRes.status === 401 || faucetRes.status === 403) {
      return NextResponse.json({
        ...
        manualInstructions: {
          ...
          step3: `Select ${CIRCLE_BLOCKCHAIN}`,
